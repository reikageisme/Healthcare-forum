...
class Report(Base):
    ...
    target_type = Column(enum_column(ReportTargetType), nullable=False, index=True)
    target_id = Column(UUID(as_uuid=True), nullable=False, index=True)
    report_type = Column(String(50), default="spam", nullable=False)
    reason = Column(String(255), nullable=False)
    details = Column(Text, nullable=True)
    status = Column(enum_column(ReportStatus), default=ReportStatus.OPEN, nullable=False, index=True)
